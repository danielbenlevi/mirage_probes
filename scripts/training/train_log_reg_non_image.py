...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train VLM probes from without-image examples labeled by without-image mirage_like, "
            "while evaluating test performance on with-image examples, "
    ...
    parser.add_argument(
        "--features_cache_path",
        type=str,
        default="./tmp_artifacts/non_image_all_examples_layer_features.pt",
    )
    parser.add_argument(
        "--force_reextract",
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default="./tmp_artifacts/non_image_all_examples_probe_results",
    )
...
def _build_dual_conversations_from_row(
    row: Dict,
    image_lookup_uid: Dict[Tuple[str, str], List[bytes]],
    image_lookup_qid: Dict[Tuple[str, str], List[bytes]],
    neutral_as_non_mirage: bool,
    ...
    conv_trainval_without_image = core._make_vllm_messages(
        prompt_text=prompt_text,
        image_bytes_list=None,
        system_prompt=system_prompt,
    )
    conv_trainval_without_image.append({"role": "assistant", "content": without_resp})

    conv_test_with_image = core._make_vllm_messages(
        prompt_text=prompt_text,
        image_bytes_list=imgs,
        system_prompt=system_prompt,
    )
    conv_test_with_image.append({"role": "assistant", "content": with_resp})
    return {
        "conversation_trainval_without_image": conv_trainval_without_image,
        "conversation_test_with_image": conv_test_with_image,
        "label": label,
...
def main() -> None:
    ...
    save_dir_path = _scope_default_path(
        Path(args.save_dir),
        vlm=str(args.vlm),
        marker="/non_image_all_examples_probe_results",
    )
    cache_path = _scope_default_path(
        Path(args.features_cache_path),
        vlm=str(args.vlm),
        marker="non_image_all_examples_layer_features.pt",
    )
    ...
    selected = [
        _build_dual_conversations_from_row(
            row,
            image_lookup_uid=image_lookup_uid,
            image_lookup_qid=image_lookup_qid,
            neutral_as_non_mirage=bool(args.neutral_as_non_mirage),
    ...
    trainval_without_image_conversations = [x["conversation_trainval_without_image"] for x in selected]
    test_with_image_conversations = [x["conversation_test_with_image"] for x in selected]
    ...
    if pair_core._uses_preextracted_activation_store(str(args.vlm)):
        ...
        (
            layer_features_test_with_image,
            layer_features_trainval_without_image,
            layer_order,
            cache_path,
        ) = pair_core._load_preextracted_all_examples_subset(
            vlm_key=str(args.vlm),
            with_conversations=test_with_image_conversations,
            without_conversations=trainval_without_image_conversations,
        ...
        if (
            "layer_features_trainval_without_image" not in payload
            or "layer_features_test_with_image" not in payload
        ):
            raise RuntimeError(
                "Cache does not contain dual conversation features required by train_log_reg_non_image.py. "
                "Re-run with --force_reextract."
            )
        layer_features_trainval_without_image = payload["layer_features_trainval_without_image"]
        layer_features_test_with_image = payload["layer_features_test_with_image"]
        ...
        if (not requested_include_additional) and cached_include_additional:
            ...
            layer_features_trainval_without_image = {
                k: v
                for k, v in layer_features_trainval_without_image.items()
                if not _is_additional_attention_mlp_feature(k)
            }
            layer_features_test_with_image = {
                k: v
                for k, v in layer_features_test_with_image.items()
                if not _is_additional_attention_mlp_feature(k)
            }
    else:
        ...
        layer_features_trainval_without_image: Dict[str, List[np.ndarray]] = defaultdict(list)
        layer_features_test_with_image: Dict[str, List[np.ndarray]] = defaultdict(list)
        layer_order: List[str] = []
        seen_layers = set()

        for conv_trainval, conv_test in tqdm(
            zip(trainval_without_image_conversations, test_with_image_conversations),
            total=len(trainval_without_image_conversations),
            desc="Extracting activations",
            unit="sample",
            dynamic_ncols=True,
        ):
            trainval_messages = pair_core._to_ovis_messages(conv_trainval)
            sample_feats_trainval = pair_core._extract_sample_features_only(
                model=model,
                messages=trainval_messages,
                include_additional_attention_mlp_probes=bool(args.include_additional_attention_mlp_probes),
                model_key=str(args.vlm),
            )
            test_messages = pair_core._to_ovis_messages(conv_test)
            sample_feats_test = pair_core._extract_sample_features_only(
                model=model,
            ...
            for key, value in sample_feats_trainval.items():
                if key not in seen_layers:
                    seen_layers.add(key)
                    layer_order.append(key)
                layer_features_trainval_without_image[key].append(value.to(torch.float32).cpu().numpy())
                layer_features_test_with_image[key].append(
                    sample_feats_test[key].to(torch.float32).cpu().numpy()
                )

        layer_order = sorted(layer_order, key=pair_core._layer_sort_key)
        torch.save(
            {
                "layer_features_trainval_without_image": layer_features_trainval_without_image,
                "layer_features_test_with_image": layer_features_test_with_image,
                "labels": labels,
                ...
                "trainval_conversation_mode": "without_image",
                "test_conversation_mode": "with_image",
            },
            cache_path,
        )
    ...
    for feature_name in tqdm(
        ...
        X_trainval = np.asarray(layer_features_trainval_without_image[feature_name], dtype=np.float32)
        X_test = np.asarray(layer_features_test_with_image[feature_name], dtype=np.float32)
        seed_runs = []
    ...
    summary_payload = {
        ...
        "training_mode": "non_image_trainval_with_image_test_benchmark_balanced_train_val_test_multiseed",
        "trainval_conversation_mode": "without_image",
        "test_conversation_mode": "with_image",
    ...
    all_features_path = save_dir / f"{vlm_tag}_non_image_all_feature_probe_accuracies.json"
    llm_strategy_path = save_dir / f"{vlm_tag}_non_image_llm_layer_best_strategy_results.json"
    sample_meta_path = save_dir / f"{vlm_tag}_non_image_sample_metadata.json"
    config_path = save_dir / f"{vlm_tag}_non_image_run_config.json"
