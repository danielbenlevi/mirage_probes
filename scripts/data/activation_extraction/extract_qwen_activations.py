...
QWEN_VLM_KEY = "qwen3_vl_32b_instruct"
FINAL_DATA_ROOT = REPO_ROOT / "data" / "final_data"
TMP_ARTIFACTS_ROOT = REPO_ROOT / "tmp_artifacts"
DEFAULT_QWEN_PAIRS_PATH = FINAL_DATA_ROOT / "qwen_contrastive.json"
DEFAULT_QWEN_RESPONSES_PATH = FINAL_DATA_ROOT / "qwen_all_responses.json"
DEFAULT_QWEN_FALLBACK_PYTHON = "python3"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Pre-extract Qwen3-VL activations once for contrastive and all-examples trainers. "
            "The resulting caches are load-only inputs for qwen trainer runs."
        )
    )
    parser.add_argument(
        "--model_path",
        type=str,
        default="",
        help="Optional explicit local model path override for Qwen3-VL.",
    )
    parser.add_argument(
        "--pairs_path",
        type=str,
        default=str(DEFAULT_QWEN_PAIRS_PATH),
    )
    parser.add_argument(
        "--neutral_pairs_path",
        type=str,
        default=str(TMP_ARTIFACTS_ROOT / "contrastive_conversation_pairs_neutral_as_non_mirage.json"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--responses_path",
        type=str,
        default=str(DEFAULT_QWEN_RESPONSES_PATH),
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Output .pt path for contrastive pre-extracted cache.",
    )
    parser.add_argument(
        "--all_examples_output_path",
        type=str,
        default="",
        help="Output .pt path for all-examples pre-extracted cache.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Write dedicated Qwen pre-extracted caches for additional probe families "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Include attention probe families in pre-extracted cache.",
    )
    parser.add_argument(
        "--include_mlp_probes",
        action="store_true",
        default=False,
        help="Include MLP probe families in pre-extracted cache.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--fallback_python",
        type=str,
        default=DEFAULT_QWEN_FALLBACK_PYTHON,
        help=(
            "Python interpreter used for automatic re-exec when current transformers "
            "cannot load qwen3_vl."
        ),
    )
    parser.add_argument(
        "--auto_python_fallback",
        dest="auto_python_fallback",
        action="store_true",
        help="Enable auto re-exec via --fallback_python when qwen3_vl is unsupported.",
    )
    parser.add_argument(
        "--no_auto_python_fallback",
        dest="auto_python_fallback",
        action="store_false",
        help="Disable auto re-exec and fail if qwen3_vl is unsupported in current env.",
    )
    parser.set_defaults(auto_python_fallback=True)
...
def _resolve_model_path(model_path: str) -> str:
    ...
    if override:
        return override
    return pair_core._default_model_path_for_vlm(QWEN_VLM_KEY)
...
def _default_contrastive_output_path() -> Path:
    return pair_core._qwen_preextracted_contrastive_path()


def _default_all_examples_output_path() -> Path:
    return pair_core._qwen_preextracted_all_examples_path()


def _default_contrastive_additional_output_path() -> Path:
    return pair_core._qwen_preextracted_contrastive_additional_path()


def _default_all_examples_additional_output_path() -> Path:
    return pair_core._qwen_preextracted_all_examples_additional_path()
...
def _transformers_has_qwen3_vl_support() -> bool:
    try:
        ...
        return bool(hasattr(transformers, "Qwen3VLForConditionalGeneration"))
    except Exception:
        return False


def _maybe_reexec_with_fallback_python(args: argparse.Namespace) -> None:
    if _transformers_has_qwen3_vl_support():
        return

    fallback_python = Path(str(args.fallback_python)).expanduser().resolve()
    if not bool(args.auto_python_fallback):
        raise RuntimeError(
            "Current transformers does not support qwen3_vl and --no_auto_python_fallback "
            f"was set. Re-run with a supported interpreter, e.g. {fallback_python}."
        )
    if not fallback_python.exists():
        raise RuntimeError(
            "Current transformers does not support qwen3_vl, and fallback python was not found: "
            f"{fallback_python}"
        )
    current_python = Path(sys.executable).resolve()
    if current_python == fallback_python:
        raise RuntimeError(
            "Current interpreter is already fallback python, but qwen3_vl is still unsupported. "
            "Install a transformers build that includes qwen3_vl in this environment."
        )

    passthrough_args = list(sys.argv[1:])
    if "--no_auto_python_fallback" not in passthrough_args:
        passthrough_args.append("--no_auto_python_fallback")
    relaunch_argv = [str(fallback_python), str(Path(__file__).resolve()), *passthrough_args]
    print(
        "[extract_qwen_activations] qwen3_vl unsupported in current transformers; "
        f"re-launching with {fallback_python}",
        flush=True,
    )
    os.execv(str(fallback_python), relaunch_argv)
...
def _extract_feature_rows(
    ...
    for signature_key, conversation in tqdm(items, desc="Extracting activations", unit="sample", dynamic_ncols=True):
        del signature_key  # carried by caller order; not needed in extraction step.
        model_messages = pair_core._to_model_messages(conversation=conversation, model_key=QWEN_VLM_KEY)
...
def _build_contrastive_items(
    ...
    return items, without_items, metadata
...
def main() -> None:
    args = parse_args()
    _maybe_reexec_with_fallback_python(args)
    ...
    if visible_gpus < int(args.min_visible_gpus):
        raise RuntimeError(
            f"Expected at least {int(args.min_visible_gpus)} visible GPUs for Qwen extraction, "
            f"but found {visible_gpus}. Set CUDA_VISIBLE_DEVICES to 4 GPUs first."
        )
    ...
    if not bool(args.skip_contrastive):
        ...
        main_pairs_path = _resolve_scoped_input_path(Path(args.pairs_path).expanduser().resolve(), QWEN_VLM_KEY)
        ...
        if not contrastive_items:
            raise RuntimeError("No contrastive conversations found for Qwen pre-extraction.")
        ...
        if contrastive_without_items:
            ...
            if contrastive_without_layer_order != contrastive_layer_order:
                raise RuntimeError(
                    "With-image and without-image layer orders differ in Qwen contrastive extraction."
                )
        contrastive_payload = {
            "cache_type": "qwen_preextracted_contrastive_features",
            "vlm": QWEN_VLM_KEY,
    ...
    if not bool(args.skip_all_examples):
        ...
        responses_path = _resolve_scoped_input_path(Path(args.responses_path).expanduser().resolve(), QWEN_VLM_KEY)
        ...
        if not with_items:
            raise RuntimeError("No with-image all-examples conversations found for Qwen pre-extraction.")
        if not without_items:
            raise RuntimeError("No without-image all-examples conversations found for Qwen pre-extraction.")
        ...
        if with_layer_order != without_layer_order:
            raise RuntimeError(
                "With-image and without-image layer orders differ in Qwen all-examples extraction."
            )

        all_examples_payload = {
            "cache_type": "qwen_preextracted_all_examples_features",
            "vlm": QWEN_VLM_KEY,
            "model_path": str(model_path),
            "feature_extraction_version": int(pair_core.FEATURE_EXTRACTION_VERSION),
            "include_attention_probes": bool(include_attention),
